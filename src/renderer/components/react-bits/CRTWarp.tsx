...
export default function CRTWarp({
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      const delta = Math.min(Math.max(now - previousRenderTime, 0) / 1000, 0.1)
  ...
  return <div ref={containerRef} className={`crt-warp-container${className ? ` ${className}` : ''}`} style={style} />
}
