  ...
  return (
    <Section className="pd-profile__metric" padding={0} variant="transparent">
      <HStack align="center" gap={3}>
        {emblem ? <Thumbnail alt={`${value} rank emblem`} className="peaks-media peaks-rank-media" label={value} src={emblem} /> : null}
        <VStack gap={2}>
...
export function PlayerProfileScreen({
  ...
  return (
    <AuthenticatedScreen
      ...
      description={`${player.region} · ${player.game ?? 'Riot profile'}${player.lastUpdated ? ` · ${player.lastUpdated}` : ''}`}
      screen="player-profile"
      title="Player profile">
      <VStack className="pd-detail pd-profile" gap={6}>
        <HStack align="center" justify="between" gap={3}>
          <Button icon={<Icon icon={ArrowLeft} />} label="Back" onClick={onBack} size="sm" variant="ghost" />
        ...
        <Grid className="pd-detail__identity" columns={2} gap={0}>
          <VStack className="pd-detail__identity-copy" gap={8} justify="between" padding={8}>
            <HStack align="center" gap={3} justify="between" wrap="wrap">
              <Text className="pd-detail__eyebrow" type="supporting">{player.game ?? 'RIOT GAMES'} / PLAYER PROFILE</Text>
              {isWatched ? <Token color="gray" label="On your watchlist" size="sm" /> : null}
            </HStack>
            <VStack gap={4}>
              <Avatar
              ...
              />
                ...
                <HStack align="center" gap={3} wrap="wrap">
                  ...
                  {player.level != null ? <Text color="secondary">Level {player.level}</Text> : null}
          ...
          <Section className="pd-detail__identity-art pd-detail__art" padding={8} variant="transparent" style={player.game ? {backgroundImage: `url("${gameArtwork(player.game, player.context?.map ?? player.matches?.[0]?.map)}")`} : undefined}>
            <VStack className="pd-detail__art-content" justify="end" align="end" gap={2}>
              <Text className="pd-detail__eyebrow" type="supporting">{player.context?.map ? 'LAST ENCOUNTER' : 'COMPETITIVE PROFILE'}</Text>
              <Heading level={3} type="display-3">{player.context?.map ? (player.game === 'VALORANT' ? valorantMapName(player.context.map) : player.context.map) : player.game ?? 'Riot Games'}</Heading>
              {player.context?.label || player.lastGame ? <Text type="supporting">{player.context?.label ?? player.lastGame}</Text> : null}
            </VStack>
          </Section>
        </Grid>
        ...
        <Grid className="pd-detail__workspace" columns={2} gap={8}>
          <VStack className="pd-detail__sidebar" gap={5}>
            <HStack align="center" gap={2}><Icon icon={Trophy} color="secondary" /><Heading level={2}>Competitive record</Heading></HStack>
            {ranks.length > 0 ? <List aria-label="Current ranks" density="balanced" hasDividers>
              {ranks.map(rank => <ListItem
                description={rankLabel(rank)}
            ...
            <ProfileMetric label="LAST SEEN" value={player.lastGame ?? player.context?.label ?? 'Unknown'} />
          </VStack>

          <VStack className="pd-detail__main" gap={6}>
            <TabList className="pd-detail__tabs" aria-label="Player profile views" hasDivider onChange={setTab} role="tablist" value={tab}>
              <Tab icon={<Icon icon={History} />} label="Activity" panelId="profile-activity-panel" value="activity" />
              <Tab icon={<Icon icon={BarChart3} />} label="Performance" panelId="profile-performance-panel" value="performance" />
            </TabList>

            {tab === 'activity' ? <VStack className="pd-detail__panel" aria-label="Activity" gap={5} id="profile-activity-panel" role="tabpanel"><MatchList heading="Recent matches" matches={player.matches ?? []} onSelect={onSelectMatch} /></VStack> : null}

            {tab === 'performance' ? <VStack className="pd-detail__panel" aria-label="Performance" gap={6} id="profile-performance-panel" role="tabpanel">
        ...
        {player.context ? (
          <Section padding={0} variant="transparent">
            <List
              ...
              density="compact"
              hasDividers
              header={<HStack align="center" justify="between" gap={3}><Heading level={2}>Match performance</Heading><Text type="supporting">{player.context.label}</Text></HStack>}>
              <ListItem
                description={`${player.context.team ?? 'Team unavailable'} · ${player.context.character ?? 'Character unavailable'}`}
        ...
        {!player.context && (player.matches ?? []).length === 0 ? <EmptyState icon={<Icon icon={BarChart3} />} title="No match data available" description="Performance details appear when matches are available for this player." /> : null}
          </VStack> : null}
          </VStack>
        </Grid>
      </VStack>
